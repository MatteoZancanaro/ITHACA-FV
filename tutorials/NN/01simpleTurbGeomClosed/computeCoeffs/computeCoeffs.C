...
class SteadyNSSimpleNN : public SteadyNSSimple
{
    public:
        ...
        void getTurbNN(word Folder = "./ITHACAoutput/Offline/")
        {
            Eigen::MatrixXd NNdata(2, 1);

            if (ITHACAutilities::check_file(Folder + "NN/NNdata.txt"))
            {
                NNdata = ITHACAstream::readMatrix(Folder + "NN/NNdata.txt");
            }

            // If the NN does not exist or input/output dimensions are changed, a new NN is constructed
            if (!ITHACAutilities::check_file(Folder + "NN/Net.pt") ||
                    NNdata(0, 0) != NUmodes || NNdata(1, 0) != NNutModes)
            {
                ...
                mkDir(Folder + "NN");
                cnpy::save(coeffL2U, Folder + "NN/coeffL2U.npy");
                cnpy::save(coeffL2Nut, Folder + "NN/coeffL2Nut.npy");
                cnpy::save(bias_inp, Folder + "NN/bias_inp.npy");
                cnpy::save(scale_inp, Folder + "NN/scale_inp.npy");
                cnpy::save(bias_out, Folder + "NN/bias_out.npy");
                cnpy::save(scale_out, Folder + "NN/scale_out.npy");
                ...
                ITHACAstream::SaveDenseMatrix(bias_inp, Folder + "NN/",
                                              "bias_inp");
                ITHACAstream::SaveDenseMatrix(scale_inp, Folder + "NN/",
                                              "scale_inp");
                ITHACAstream::SaveDenseMatrix(bias_out, Folder + "NN/",
                                              "bias_out");
                ITHACAstream::SaveDenseMatrix(scale_out, Folder + "NN/",
                                              "scale_out");
                std::ofstream NNFile;
                NNFile.open (Folder + "NN/NNdata.txt");
            ...
            {
                torch::load(Net, Folder + "NN/Net.pt");
                ITHACAstream::ReadDenseMatrix(bias_inp, Folder + "NN/",
                                              "bias_inp");
                ITHACAstream::ReadDenseMatrix(scale_inp, Folder + "NN/",
                                              "scale_inp");
                ITHACAstream::ReadDenseMatrix(bias_out, Folder + "NN/",
                                              "bias_out");
                ITHACAstream::ReadDenseMatrix(scale_out, Folder + "NN/",
                                              "scale_out");
            }
        }
...
int main(int argc, char* argv[])
{
    ...
    tutorial01cl train(argc, argv);
    // Read some parameters from file
    ITHACAparameters* para = ITHACAparameters::getInstance(train._mesh(),
                             train._runTime());
    ITHACAstream::readMiddleFields(train.Ufield, train._U(),
                                   "./ITHACAoutput/Offline/");
    ITHACAstream::readMiddleFields(train.Pfield, train._p(),
                                   "./ITHACAoutput/Offline/");
    auto nut_train = train._mesh().lookupObject<volScalarField>("nut");
    ITHACAstream::readMiddleFields(train.nutFields, nut_train,
                                   "./ITHACAoutput/Offline/");
    ITHACAPOD::getModes(train.Ufield, train.Umodes, train._U().name(),
                        train.podex, 0, 0,
                        0);
    ITHACAPOD::getModes(train.Pfield, train.Pmodes, train._p().name(),
                        train.podex, 0, 0,
                        0);
    ITHACAPOD::getModes(train.nutFields, train.nutModes, "nut",
                        train.podex, 0, 0, 0);
    // Construct the test object
    tutorial01cl test(argc, argv);
    ITHACAstream::readMiddleFields(test.Ufield, test._U(),
                                   "./ITHACAoutput/checkOff/");
    ITHACAstream::readMiddleFields(test.Pfield, test._p(),
                                   "./ITHACAoutput/checkOff/");
    auto nut_test = test._mesh().lookupObject<volScalarField>("nut");
    ITHACAstream::readMiddleFields(test.nutFields, nut_test,
                                   "./ITHACAoutput/checkOff/");
    // Compute the coefficients
    Eigen::MatrixXd coeffL2Nut_train = ITHACAutilities::getCoeffs(train.nutFields,
                                       train.nutModes,
                                       0, true);
    coeffL2Nut_train.transposeInPlace();
    Eigen::MatrixXd coeffL2P_train = ITHACAutilities::getCoeffs(train.Pfield,
                                     train.Pmodes,
                                     0, true);
    coeffL2P_train.transposeInPlace();
    Eigen::MatrixXd coeffL2U_train = ITHACAutilities::getCoeffs(train.Ufield,
                                     train.Umodes,
                                     0, true);
    coeffL2U_train.transposeInPlace();
    Eigen::MatrixXd bias_inp_train_U  = coeffL2U_train.colwise().minCoeff();
    Eigen::MatrixXd scale_inp_train_U = coeffL2U_train.colwise().maxCoeff() -
                                        coeffL2U_train.colwise().minCoeff();
    Eigen::MatrixXd bias_inp_train_P  = coeffL2P_train.colwise().minCoeff();
    Eigen::MatrixXd scale_inp_train_P = coeffL2P_train.colwise().maxCoeff() -
                                        coeffL2P_train.colwise().minCoeff();
    Eigen::MatrixXd bias_out_train  = coeffL2Nut_train.colwise().minCoeff();
    Eigen::MatrixXd scale_out_train = coeffL2Nut_train.colwise().maxCoeff() -
                                      coeffL2Nut_train.colwise().minCoeff();
    mkDir("train");
    cnpy::save(coeffL2U_train, "train/coeffL2U.npy");
    cnpy::save(coeffL2P_train, "train/coeffL2P.npy");
    cnpy::save(coeffL2Nut_train, "train/coeffL2Nut.npy");
    cnpy::save(bias_inp_train_U, "train/bias_inp_U.npy");
    cnpy::save(scale_inp_train_U, "train/scale_inp_U.npy");
    cnpy::save(bias_inp_train_P, "train/bias_inp_U.npy");
    cnpy::save(scale_inp_train_P, "train/scale_inp_U.npy");
    cnpy::save(bias_out_train, "train/bias_out.npy");
    cnpy::save(scale_out_train, "train/scale_out.npy");
    // Compute the coefficients
    Eigen::MatrixXd coeffL2Nut_test = ITHACAutilities::getCoeffs(test.nutFields,
                                      train.nutModes,
                                      0, true);
    coeffL2Nut_test.transposeInPlace();
    Eigen::MatrixXd coeffL2P_test = ITHACAutilities::getCoeffs(test.Pfield,
                                     train.Pmodes,
                                     0, true);
    coeffL2P_test.transposeInPlace();
    Eigen::MatrixXd coeffL2U_test = ITHACAutilities::getCoeffs(test.Ufield,
                                    train.Umodes,
                                    0, true);
    coeffL2U_test.transposeInPlace();
    Eigen::MatrixXd bias_inp_test_U  = coeffL2U_test.colwise().minCoeff();
    Eigen::MatrixXd scale_inp_test_U = coeffL2U_test.colwise().maxCoeff() -
                                       coeffL2U_test.colwise().minCoeff();
    Eigen::MatrixXd bias_inp_test_P  = coeffL2P_test.colwise().minCoeff();
    Eigen::MatrixXd scale_inp_test_P = coeffL2P_test.colwise().maxCoeff() -
                                       coeffL2P_test.colwise().minCoeff();
    Eigen::MatrixXd bias_out_test  = coeffL2Nut_test.colwise().minCoeff();
    Eigen::MatrixXd scale_out_test = coeffL2Nut_test.colwise().maxCoeff() -
                                     coeffL2Nut_test.colwise().minCoeff();
    mkDir("test");
    cnpy::save(coeffL2U_test, "test/coeffL2U.npy");
    cnpy::save(coeffL2P_test, "test/coeffL2P.npy");
    cnpy::save(coeffL2Nut_test, "test/coeffL2Nut.npy");
    cnpy::save(bias_inp_test_U, "test/bias_inp_U.npy");
    cnpy::save(scale_inp_test_U, "test/scale_inp_U.npy");
    cnpy::save(bias_inp_test_P, "test/bias_inp_P.npy");
    cnpy::save(scale_inp_test_P, "test/scale_inp_P.npy");
    cnpy::save(bias_out_test, "test/bias_out.npy");
    cnpy::save(scale_out_test, "test/scale_out.npy");
    exit(0);
}
