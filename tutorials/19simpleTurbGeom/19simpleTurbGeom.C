...
class SteadyNSSimpleNN : public SteadyNSSimple
{
    public:
        ...
        void getTurbNN()
        {
            ...
            if (!ITHACAutilities::check_file("./ITHACAoutput/Offline/NN/Net.pt") || NNdata(0,0) != NUmodes || NNdata(1,0) != NNutModes)
            {
                ...
                Net->push_back(torch::nn::Linear(NUmodes, 512));
                Net->push_back(torch::nn::ReLU());
                Net->push_back(torch::nn::Linear(512, 256));
                Net->push_back(torch::nn::ReLU());
                Net->push_back(torch::nn::Linear(256, 128));
                Net->push_back(torch::nn::ReLU());
                Net->push_back(torch::nn::Linear(128, 64));
                Net->push_back(torch::nn::ReLU());
                Net->push_back(torch::nn::Linear(64, 32));
                Net->push_back(torch::nn::ReLU());
                Net->push_back(torch::nn::Linear(32, NNutModes));
                optimizer = new torch::optim::Adam(Net->parameters(),
                                                   torch::optim::AdamOptions(2e-3));
                int epochs = 15000;
...
class reducedSimpleSteadyNN : public reducedSimpleSteadyNS
{
    ...
    Eigen::VectorXd a0 = ITHACAutilities::getCoeffs(U, problem->Umodes, 0, false);
    a(0) = a0(0);
...
class tutorial19 : public SteadyNSSimpleNN
{
    public:
        /// Constructor
                          ...
                          word folder = "./ITHACAoutput/Offline/")
        {
            ...
            if (offline && ITHACAutilities::isTurbulent() && !ITHACAutilities::check_folder("./ITHACAoutput/POD/1"))
            {
...
int main(int argc, char* argv[])
{
    ...
    std::cout << "SONO PROPRIO IO" << std::endl;
    ...
    projUfullTest.resize(UfullTest.size());
    projPfullTest.resize(PfullTest.size());
    // std::cout << UfullTest.size() << std::endl;
    // std::cout << projUfullTest.size() << std::endl;
    std::cerr << "modiU: " << example.Umodes.size() << std::endl;
    example.Umodes.projectSnapshots(UfullTest,projUfullTest,example.NUmodes,"L2");
    std::cerr << "debug point 2" << std::endl;
    example.Pmodes.projectSnapshots(PfullTest,projPfullTest,example.NPmodes,"L2");
    Eigen::MatrixXd projUErr = ITHACAutilities::errorL2Rel(UfullTest, projUfullTest);
    Eigen::MatrixXd projPErr = ITHACAutilities::errorL2Rel(PfullTest, projPfullTest);
    exit(0);
