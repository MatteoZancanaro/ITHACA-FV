...
int main(int argc, char* argv[])
{
    ...
    ITHACAPOD::getModes(example.Ufield, example.Umodes, example.podex, 0, 0, 50);
    ITHACAPOD::getModes(example.Pfield, example.Pmodes, example.podex, 0, 0, 50);
    // ITHACAPOD::getModes(example.Ufield, example.Umodes, example.Volumes,
    ...
    //                               name(1) + "/polyMesh/");
    //    reduced.setOnlineVelocity(vel);
    // reduced.solveOnline_Simple(1, 12,10, "./ITHACAoutput/checkOn/");

    //exit(0);

    for (int i = 0; i < 50; i++)
    {
        example.updateMesh(-4.0);
        reduced.setOnlineVelocity(vel);
        ITHACAstream::writePoints(example._mesh().points(), "./ITHACAoutput/checkOn",
                                  name(i + 1) + "/polyMesh/");
        reduced.solveOnline_Simple(1, i + 1, i + 1, "./ITHACAoutput/checkOn/");
    }
