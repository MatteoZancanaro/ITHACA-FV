...
## WING Parameters
np_naca = 400			# Only even numbers, number of points along the naca
angle = 0 			# Angle of attack of the naca
naca_type = '4412'		# Naca type (pos. choiches 2414 , 0009 , 6409)
scale = 1		# Dimension of the chord

## Geometrical properties of the mesh
# X location of upper wing point with respect to the chord length (0 is on the leading edge, 1 on the trailing edge)
Xup = 0.5
# X location of lower wing point with respect to the chord length (0 is on the leading edge, 1 on the trailing edge)
Xlo = 0.5
...
xp08 = xupr[-1]
yp08 = zupr[-1]
xp09 = xupr[int(len(xupr)*(1-Xup))]
yp09 = zupr[int(len(zupr)*(1-Xup))]
xp11 = xupr[0]
yp11 = zupr[0]
xp10 = xlor[int(len(xlor)*Xlo)]
yp10 = zlor[int(len(zlor)*Xlo)]
plt.plot(xupr,zupr)
plt.plot(xlor,zlor)
plt.plot(xp08,yp08,"*")
plt.plot(xp09,yp09,"*")
