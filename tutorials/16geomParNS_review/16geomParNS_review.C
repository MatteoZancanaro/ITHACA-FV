...
int main(int argc, char* argv[])
{
    ...
    std::ifstream exFile("./parsOff_mat.txt");

    if (exFile)
    {
        parOff = ITHACAstream::readMatrix("./parsOff_mat.txt");
    }

    else
    {
        parOff = ITHACAutilities::rand(100, 10, 0, 0.02);
        ITHACAstream::exportMatrix(parOff, "pars", "eigen", "./");
    }
    Eigen::MatrixXd parTop = parOff.leftCols(5);
    Eigen::MatrixXd parBot = parOff.rightCols(5)*(-1);

    example.OfflineSolve(parTop, parBot, "./ITHACAoutput/Offline/");
    ITHACAstream::read_fields(example.liftfield, example.U, "./lift/");
    std::cout << example.liftfield.size() << std::endl;
    ...
    {
        parOn = ITHACAutilities::rand(50, 10, 0, 0.02);
        ITHACAstream::exportMatrix(parOn, "pars", "eigen", "./");
    }
    Eigen::MatrixXd parTopOn = parOn.leftCols(5);
    Eigen::MatrixXd parBotOn = parOn.rightCols(5)*(-1);

    for (int i = 0; i < parTopOn.rows(); i++)
    {
        checkOff.updateMesh2(parTopOn.row(i),parBotOn.row(i));
        /// Offline part;
